 ...
 * @example
 * // return xUserId
 * transformHeaderName(':X-User-Id')
 * @param {*} name Http Header Name
 */
const transformHeaderName = function (name) {
  const res = name
    .split(/[-_:]/)
    .filter(filterEmpty)
    .map(function (key) {
...
const getDefinitionName = function (name) {
  const valids = name.replace(/#\/definitions\//, '').match(/[a-zA-Z\d$\u4e00-\u9fa5]/g)
  return 'I' + (valids && valids.length ? valids.join('') : name);
};
...
const tsByDefinition = function (name, definition) {
  ...
  if (definition.type === 'object') {
    ...
    if (!definition.properties) {
      ...
      return `export interface ${inerfaceName} {
  [key: string]: any;
}`
    }
    ...
    Object.keys(properties).forEach(function (prop) {
      ...
      if (isEnum) {
        ...
        enumDefinitions.push(`type ${enumName} = ${properties[prop].enum.map(e => `'${e}'`).join(' | ')};\n`);
      }
  ...
  const codeBody = apis
    .map((api, apiIndex) => {
      ...
      const headersWithValidNameParams = headerParams.map(function (p) {
        const hName = p.name;
        const transformed = transformHeaderName(hName);
      ...
      const returns = `  return http.${api.method}(${[url, configParamState]
        .filter(filterEmpty)
        .join(', ')});`;
      ...
      const argsStatement = [
        ...
        bodyAndFormParams.length ? `data${(finalParamOptional.body || finalParamOptional.formData) ? '' : '?'}: {\n  ${paramTypes.body}\n  }` : '',// spreadArg(bodyAndFormParams),
      ...
      return `
${comment}export const ${methodName} = function(\n  ${argeStatements}\n): ${`Promise<${withDataType}>`} {
${[returns].filter(filterEmpty).join('\n')}
};
`;
    })
    .join('');
  ...
  // Object.keys(AllResponseDep).forEach(name => {
  //   tsImports.push(`import ${name} from '${typedefBase}/${name}'\n`);
  // })
